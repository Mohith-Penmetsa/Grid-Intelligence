      ...
      <div className="overflow-x-auto">
        <table className="w-full text-sm text-left">
          <thead className="text-[11px] uppercase tracking-wider text-muted-foreground bg-surface-3/50 border-b border-border/50">
            <tr>
              <th className="px-3 py-3 font-medium whitespace-nowrap min-w-[110px]">Transformer ID</th>
              <th className="px-3 py-3 font-medium">Area</th>
              <th className="px-3 py-3 font-medium text-right">Input</th>
              <th className="px-3 py-3 font-medium text-right">Consumption</th>
              <th className="px-3 py-3 font-medium text-right">Tech Loss</th>
              <th className="px-3 py-3 font-medium text-right">Comm Loss</th>
              <th className="px-3 py-3 font-medium text-right">Comm Loss %</th>
          ...
          <tbody className="divide-y divide-border/50">
            {data.map((analysis) => {
              ...
              return (
                ...
                >
                  <td className="px-3 py-3 font-medium whitespace-nowrap text-foreground">{t.id}</td>
                  <td className="px-3 py-3 text-muted-foreground min-w-[120px]">{t.area}</td>
                  <td className="px-3 py-3 text-right">{t.input}</td>
                  <td className="px-3 py-3 text-right">{t.consumed}</td>
                  <td className="px-3 py-3 text-right text-muted-foreground">{t.techLoss}</td>
                  <td className="px-3 py-3 text-right font-medium text-amber-400">{t.commLoss}</td>
                  <td className="px-3 py-3 text-right font-bold text-amber-400">{analysis.commercialLossPercentage.toFixed(1)}%</td>
                  <td className="px-3 py-3 text-center">
                    <div className="flex items-center justify-center gap-2">
                      <div className="w-12 h-1.5 bg-muted rounded-full overflow-hidden">
                        <div 
                          className={cn("h-full rounded-full", (isCritical || isHigh) ? "bg-risk-critical" : (isMedium ? "bg-amber-400" : "bg-emerald-400"))}
                  ...
                  <td className="px-3 py-3">
                    <Badge variant="outline" className={cn(
                      ...
                      (isCritical || isHigh) ? "border-risk-critical text-risk-critical bg-risk-critical/10" : 
